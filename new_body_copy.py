...
import tensorforce
import tensorflow
from new_bank_copy import prediction_environment_new
...
result_location_step = result_location + data_source + current_time + 'A2C.csv'


new_states_address='C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\3rd_predictor\\db\\mydb_01.db'
...
agent = Agent.create(agent='a2c', environment=env, memory=60000, batch_size=10, learning_rate=1e-3)
...
for mix in range(5000):
    ...
    trainX_listed, trainY_listed = new_bank_copy.listed_data_selector_opt_new (current_24_h, From_i, dataset, train_window,
    ...
    # print (1)
    trainX, trainY, actions,reward, case = new_bank_copy. RL_BASED_TRAINING_DATA_SELECTOR (trainX_listed, trainY_listed, env_prediction, agent, new_states)
    ...
    lower, upper = new_bank_copy.ONE_STEP_AHEAD_predictor(current_24_h, trainX, trainY,  train_window)
    fb_svr = new_bank_copy.Evaluation_3(real_data_current, real_data_last, real_data_last_X_2, lower_p, upper_p, lower, upper, mix)
    lower_p = lower[0]
    upper_p = upper[0]
