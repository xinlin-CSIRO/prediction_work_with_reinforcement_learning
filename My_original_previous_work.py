...
label_=pd.read_csv(Path_sorce_, header=0, usecols=[1])
...
result_location_step = result_location + 'original_UCI_' + current_time + '.csv'
...
print("Table dropped... ")
sql_create_projects_table =  "CREATE TABLE prediction ( id integer PRIMARY KEY,   real_load string, X_ross string, Y_ross string,  X string, Y string,  Extracted string, Labels string, break_index integer, scenario integer); "
...
for mix in range(5000):
        ...
        if(scenario==1):
                if(_x_>-500):
                    if(mix>analysis_back_steps):
                       sqlite_query__ = """select scenario from  prediction where id<=(?) and id> (?)"""
            ...
            sqlite_query__ = """select real_load from  prediction where id=?"""
    ...
    sqlite_insert_with_param = """INSERT INTO prediction
