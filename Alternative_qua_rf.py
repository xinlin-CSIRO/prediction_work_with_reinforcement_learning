...
#nLevels = 10
lei_ji=0
result_location="C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\3rd_predictor\\Qutail_rf_"
now = datetime.now()
...
for mix in range(5000):
    ...
    qrf.fit(trainX, trainY)
    current_24_h = np.reshape(current_24_h, (current_24_h.shape[1], current_24_h.shape[0]))
    y_pred = qrf.predict(current_24_h, quantiles=[0.3, 0.7])
    prediction_result = scaler.inverse_transform(y_pred)
