...
def listed_data_selector_opt_new (for_test, i_tern,dataset,train_window, prediction_window):
    ...
    for i in range(0, data_capacility-1):  # find the most close 7 series
        ...
        candidate_train = train_one_year[location]
        candidate_test = test_one_year[location]
        # plt.plot(candidate_train,  linewidth=1)
        train_list.append(candidate_train)
        # print (candidate_train)
        test_list.append(candidate_test)
...
class prediction_environment_new (Environment):
    def __init__(self,new_states):
        super().__init__()
        self.path=new_states
        # self.observations=98
    ...
    def execute(self, actions):
        ...
        sqlite_query__ = """select * from  new_state"""
        cur.execute(sqlite_query__)
        rows = cur.fetchall()
        state_=np.array(rows)
        # print (state_)
...
def reward_function_new(state):
    ...
    q1,q3 = state[0,4],state[0,5]

    dynamic_chang_load=real-last_real
    if (q1<= dynamic_chang_load <= q3):
        load_changing_index = 0
    else:
        load_changing_index = 1
    ...
    if(low <= real <=high):
        ...
        if   (load_changing_index==1) and (dynmiac_change_interval >= feasible_index):# inevitable--> change is so big and the interval has already been big enough
            rewards = 0
            case = 3
        elif (load_changing_index==1) and (dynmiac_change_interval < feasible_index):  #  evitable--> change is so big  but interval has not been big enough
            a = 10* abs(mid - real)/real #没有选对合适的数据
            rewards = -a
            case = 4
        elif (load_changing_index==0) and (dynmiac_change_interval > feasible_index):  #  evitable--> change is not big but interval has been big enough
            a =  abs(mid - real) / real #预测器不准确，但是不是数据的锅
            rewards = -a
            case = 5
        elif (load_changing_index==0) and (dynmiac_change_interval < feasible_index):  #  evitable--> change isnot so big but interval also has not been big enough
            a = 10*abs(mid - real) / real #没有选对合适的数据
            rewards = -a
            case = 6
...
def Evaluation_3 (real_data_current, real_data_last, real_data_last_X_2, lower_p, upper_p, lower, upper, mix):
    ...
    connection = sqlite3.connect('C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\3rd_predictor\\db\\mydb_01.db')
    cur = connection.cursor()
    ...
    if(mix==0):
        ...
        if (mix > analysis_back_steps):
            ...
            current_ = np.array([real_data_current, current_load_diff, upper[0], lower[0]])
